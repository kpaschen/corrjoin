...
func TestExtendTimeline(t *testing.T) {
	...
	knownTimeseries := make(map[int]bool)

	// Base case: appending data for the first stride.
	resp = explorer.extendTimeline(resp, map[int]float32{1: 0.9, 2: 0.8}, knownTimeseries, explorer.strideCache[0])
	...
	resp = explorer.extendTimeline(resp, map[int]float32{1: 0.95, 2: 0.88, 3: 0.9}, knownTimeseries, explorer.strideCache[1])
	if len(resp) != 6 {
	...
	resp = explorer.extendTimeline(resp, map[int]float32{2: 0.89, 3: 0.91}, knownTimeseries, explorer.strideCache[2])
	if len(resp) != 9 {
